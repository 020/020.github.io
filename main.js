    ...
    var default_defn = { icon: "fa-book" }
    var ckan_url = "https://catalogue.data.wa.gov.au"
    
    var theme_defns = {
        "Business Support and Regulation": { icon: "fa-book" },
        "Cultural Affairs": { icon: "fa-paint-brush" },
        "Defence": { icon: "fa-fighter-jet" },
        "Environment": { icon: "fa-leaf" },
        "Finance Management": { icon: "fa-dollar-sign" },
        "Immigration": { icon: "fa-plane" },
        "Maritime Services": { icon: "fa-anchor" },
        "Security": { icon: "fa-shield-alt" },
        "Trade": { icon: "fa-ship" },
        "Civic Infrastructure": { icon: "fa-road" },
        "Indigenous Affairs": { icon: "fa-bullseye" },
        "Natural Resources": { icon: "fa-gem" },
        "Sport and Recreation": { icon: "fa-futbol" },
        "Transport": { icon: "fa-train" },
        "Communications": { icon: "fa-phone" },
        "Education and Training": { icon: "fa-graduation-cap" },
        "Governance": { icon: "fa-university" },
        "International Relations": { icon: "fa-globe" },
        "Primary Industries": { icon: "fa-industry" },
        "Statistical Services": { icon: "fa-chart-bar" },
        "Community Services": { icon: "fa-wheelchair" },
        "Employment": { icon: "fa-users" },
        "Health Care": { icon: "fa-syringe" },
        "Justice Administration": { icon: "fa-balance-scale" },
        "Science": { icon: "fa-flask" },
        "Tourism": { icon: "fa-suitcase" },
    }
    
    function get_package_counts_by_facet(facet_name) {
        return $.ajax({
            url: ckan_url + "/api/3/action/package_search",
            data: { "facet.field": '["' + facet_name + '"]', rows: 0 },
            type: "GET",
            dataType: "json",
        })
    }
    ...
    $.when(get_package_counts_by_facet("theme"), get_package_counts_by_facet("geospatial_theme")).done(function (themes, geospatial_themes) {
        ...
        Object.keys(all_theme_counts)
            ...
            .forEach(function (theme_name) {
                ...
                var html = $("<p>", { class: "category_item" })
                    .append($("<i>", { class: "fas fa-fw mr-2 " + theme_defn.icon }))
                    .append($("<a>", { href: ckan_search_url }).text(theme_name))
    ...
    function searchType() {
        ...
        switch (form.search_type.selectedIndex) {
            ...
            case 4:
                ...
                //Zendesk API
                //https://toolkit.data.wa.gov.au/api/v2/help_center/articles/labels.json
                //needs authentication expose credentials?
                break;
        }
    }
